...
bit pbFlag = 0;						// Initialize interrupt flag
...
unsigned char multiplier;			// Multiplier for detracting score
...
void main (void)
{
	...
	while (1)
	{
		score = 0; 						// Init score to equal 0
		time_sum = 0;					// Init time sum to equal 0
		multiplier = 1;					// Init multiplier to equal 1, no penalty
		printf("\033[2J");				// Erase screen and move cursor to the home posiiton.
		for(i=0; i<5; ++i){
			random_number = random();
			random_number = random_number * 7200;
			pbFlag = 0;
			// Wait for a random interval [1, 5] seconds & GO!
			while (count < random_number) {
				// Bad score multiplier goes up if they push the button early
				if (pbFlag == 1) {
					multiplier += 1;
				}
				pbFlag = 0;
			...
			while (!pbFlag);
			reaction_time = count;
			reaction_time = reaction_time/7200.0;
			score += (count*multiplier);
			time_sum += reaction_time;
			printf_fast_f("%0.5f\r\n", reaction_time);
			count = 0;
		}
		printf("\r\nYour total score was %d\r\n", score);
		printf_fast_f("With avg time %0.5f\r\n", time_sum/5);
		pbFlag = 0;
...
//
void PB_ISR (void) interrupt 0		// Interrupt 0 corresponds to vector address 0003h.
...
void TIMER_INIT(void)
{
	...
	CKCON  &= ~0x08;				// Uses SYSCLK/12 as source
	TMOD   &= 0xF0;					// Clear the 4 least bits
	TMOD   |= 0x02;					// Timer0 in mode 1
	//TR0     = 0;					// Stop Timer0
...
// returns random integer between 0 and 5. 
unsigned char random(void) {
	return (rand()%5 + 1); 				// rand returns random number between 0 and 32767
